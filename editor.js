...
Game.update = function () {
	...
	for (i = Game.BankAchievements.length - 1; i >= 0; i--) {
	...
	for (i = Game.ObjectsById.length - 1; i >= 0; i--) {
		...
		if (!Game.HighestBuilding && building.amount > 0) {
...
Game.updateGrandmapocalypse = function () {
	...
	var popCookiesTitle = popCookies > 0 ? "+" + Game.BeautifyAbbr(popCookies, 1, true) + popPlural : "";
	$("#popWrinklers").toggleClass("hidden", !Game.numWrinklers).attr("data-title", popCookiesTitle);
	$("#cooksMunchedAdd").html("(+" + Game.formatNumber(popCookies, 1, true) + popPlural + ")")
...
Game.CalculateGains = function () {
	...
	Game.rawMult = mult;
	for (i = Game.CpsAchievements.length - 1; i >= 0; i--) {
	...
	Game.cpsSucked = wrinklers * suckRate;
...
Game.importSave = function (str) {
	...
	for (var i = Game.prefsSaveOrder.length - 1; i >= 0; i--) {
	...
	if (version === 1.9) { //are we importing from the 1.9 beta? remove all heavenly upgrades and refund heavenly chips
		for (i = Game.UpgradesByPool.prestige.length - 1; i >= 0; i--) {
